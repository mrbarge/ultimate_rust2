...
fn main() {
    ...
    let handle_c = thread::spawn(move || {
        for v in rx3.iter() {
            info!("Handler C got {}", v);
        }
    });
    let handle_d = thread::spawn(move ||{
        for v in rx4.iter() {
            info!("Handler D got {}", v);
        }
    });

    for v in vec!["abc","def","ghi","jkl","mno","pqr","stu","vwx","yz"] {

        tx3.send(v).unwrap()
    }
    drop(tx3);

    handle_c.join().expect("thread C should finish");
    handle_d.join().expect("thread D should finish");
